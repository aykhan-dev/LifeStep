...
private const val FRIENDS_STARTING_PAGE_INDEX = 1
...
class FriendsPagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, FriendContentPOJO> {
        val position = params.key ?: FRIENDS_STARTING_PAGE_INDEX
        return try {
            val response = service.getFriendsList(token, lang, position)
            ...
            if (response.isSuccessful) {
                if (response.body() != null) {
                    ...
                    if (body.status.code != 300) {
                        ...
                        LoadResult.Page(
                            data = items,
                            prevKey = if (position == FRIENDS_STARTING_PAGE_INDEX) null else position - 1,
